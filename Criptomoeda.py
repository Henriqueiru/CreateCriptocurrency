...
def mine_block():
    ...
    response = {'message': 'Parabéns você minerou um bloco!', 'index': block['index'], 'timestamp': block['timestamp'], 'proof': block['proof'], 'previous_hash': block['previous_hash'],'transaction': block['transactions']}
...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message': ' O blockchain é invalido '}
    return jsonify(response), 200
...
def add_transaction():
    ...
    response = {'message': f'Esta transação sera adicionada ao bloco {index}'}
...
def connect_node(): 
    ...
    response = {'message': 'Todos nos conectados, blockchain contem os seguintes nos:',
                'tortal_nodes': list(blockchain.nodes)}
    return jsonify(response), 201
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'Os nos tinham cadeias diferentes entao foi substituida',
